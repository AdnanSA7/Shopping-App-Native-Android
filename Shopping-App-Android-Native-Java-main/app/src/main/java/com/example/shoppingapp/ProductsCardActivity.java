    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new ProductAdapter(products, new OnRecyclerViewClickListener() {
            ...
            public void OnItemClick(int productId) {
                ...
                // Launch DisplayProductsActivity and wait for a result
...
//                startActivity(i);
    ...
    // Override onActivityResult to refresh data
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);

        if (requestCode == 100 && resultCode == RESULT_OK) {
            // Reload updated product list from the database
            ArrayList<Products> updatedProducts = db.getAllProducts(table_name);

            // Update adapter with new data
            adapter.setProducts(updatedProducts);
            adapter.notifyDataSetChanged();
        }
    }
