...
public class ShoppingDatabase extends SQLiteOpenHelper {
    ...
    public ArrayList<Products> getAllProducts(String tableName){
        ...
        if(cursor.moveToFirst()){
            do{
                @SuppressLint("Range") int id = cursor.getInt(cursor.getColumnIndex(TB_CLM_ID));  // Retrieve the ID
...
//        Log.d("getProduct", "Product ID: " + product_id);
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + tableName + " WHERE " + TB_CLM_ID + " = ?", new String[]{String.valueOf(product_id)});
...
//            db.close();
            ...
            Log.e("getProduct", "Product not found with ID: " + product_id);
            db.close();
            return null;
        }
    }
...
//        ContentValues values = new ContentValues();
//
//        Products product = getProduct(p.getId(),"game");
//        if(product.getQuantity()>p.getQuantity()){
//            newQuantity = product.getQuantity()-p.getQuantity();
...
//
//            updateProductQuantity(p.getId(), "game", newQuantity);
//            db.close();
//            return res != -1;
//        }else{
//            return false;
...
//    }
    ...
    public boolean insertProductInPurchases(Products p) {
        ...
        // Log the product ID to debug if it's being set correctly
        Log.d("insertProductInPurchases", "Product ID: " + p.getId());

        // Ensure the product ID is not 0 before proceeding
//        if (p.getId() == 0) {
//            Log.e("insertProductInPurchases", "Invalid Product ID: 0");
//            db.close();
//            return false;  // Return false if the product ID is invalid
//        }

        // Retrieve the current product from the "game" table
        Products product = getProduct(p.getId(), "game");
        System.out.println("Product Quantity: "+product.getPieces());

        // Check if the product is found in the database
//        if (product == null) {
//            Log.e("insertProductInPurchases", "Product not found with ID: " + p.getId());
//            db.close();
//            return false;  // Return false if the product doesn't exist
//        }

        // Check if there is enough stock
        if (product.getPieces() >= p.getQuantity()) {
            // Calculate the new quantity after the purchase
            int newQuantity = product.getPieces() - p.getQuantity();
            ...
            values.put(TB_CLM_BRAND, p.getBrand());
            values.put(TB_CLM_RATING, p.getRating());
            values.put(TB_CLM_QUANTITY, p.getQuantity());  // Store the purchased quantity

            // Insert into purchases table
            long res = db.insert(TB_PURCHASES, null, values);

            // Update the product quantity in the "game" table
            boolean updated = updateProductQuantity(p.getId(), "game", newQuantity);

//            getAllProducts("game");

            // Close the database and return the result
//            db.close();
            return res != -1 && updated;
        } else {
            db.close();
            Log.e("insertProductInPurchases", "Not enough stock for product ID: " + p.getId());
...
//    update product
public boolean updateProductQuantity(int product_id, String tableName, int newQuantity) {
    SQLiteDatabase db = getWritableDatabase();  // Use writable database to update

    // Prepare the content values with only the quantity (pieces)
    ContentValues contentValues = new ContentValues();
    contentValues.put(TB_CLM_PIECES, newQuantity);  // Assuming TB_CLM_PIECES is the column for quantity

    // Update the product where the ID matches the given product_id
    int rowsUpdated = db.update(tableName, contentValues, TB_CLM_ID + " = ?", new String[]{String.valueOf(product_id)});

//    db.close();

    // Return true if the update was successful (rowsUpdated > 0)
